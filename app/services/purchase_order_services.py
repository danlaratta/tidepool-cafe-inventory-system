...
class PurchaseOrderService:
    ...
    async def create_new_purchase_order(self, order_status: PurchaseOrderStatus, updated_at: datetime | None, delivery_date: datetime | None, supplier_id: int) -> PurchaseOrder:
        # Get supplier
    ...
    async def update_purchase_order(self, order_id: int, status: PurchaseOrderStatus, updated_at: datetime | None, delivery_date: datetime | None) -> PurchaseOrder:
        ...
        if updated_at is not None and updated_at < datetime.now():
            raise ValueError('Updated at must be a current or future date, cannot be updated to a date in the past')
